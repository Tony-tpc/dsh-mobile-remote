...
.collapsebtn{align-self:flex-start;font-size:11px;color:#8a97a8;background:#1a1f27;border:1px solid #232c38;border-radius:8px;padding:3px 9px;cursor:pointer}
.collapsebtn:active{background:#1b232d}
...
export function apply(ctx, config) {
  ...
  ctx.on('session/event', (session, event) => {
    if (event.type === 'approval/asked') {
      pendingApprovals.set(event.data.id, {
        id: event.data.id,
  ...
  const approvalList = () => {
    ...
    for (const a of pendingApprovals.values()) {
      out.push({ id: String(a.id), sessionId: a.sessionId, toolName: a.toolName, reason: a.reason, time: a.time })
    }
    return out
  }
