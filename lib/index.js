...
.activity{max-height:50vh;overflow-y:auto;display:flex;flex-direction:column;gap:6px}
.act{padding:9px 10px;border-radius:10px;background:#151b23;border:1px solid #1a212b}
.act .k{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.4px}
.act.user .k{color:#60a5fa}.act.assistant .k{color:#34d399}.act.tool .k{color:#c4b5fd}
.act .m{margin-top:6px;font-size:10px;color:#5b6470}
...
<main>
  ...
  <div class="card"><h2><span class="ic">🕘</span>最近动态</h2><div class="activity" id="activity"><div class="empty">—</div></div></div>
...
<script>
var token='';var selected=null;var sessions=[];
...
function renderActivity(d){var el=document.getElementById('activity');if(!d.activity||!d.activity.length){el.innerHTML='<div class="empty">暂无动态</div>';return;}var h='';for(var i=0;i<d.activity.length;i++){var a=d.activity[i];var k=a.kind==='user'?'你':(a.kind==='assistant'?'助手':'工具');var tm=a.time?new Date(a.time).toLocaleTimeString():'';h+='<div class="act '+a.kind+'"><div class="k">'+k+'</div><div class="md">'+a.html+'</div><div class="m">'+tm+'</div></div>';}el.innerHTML=h;el.scrollTop=el.scrollHeight;}
...
export function apply(ctx, config) {
  ...
  const detail = async (sessionId) => {
    ...
    const activity = []
    for (let i = events.length - 1; i >= 0 && activity.length < 24; i--) {
      const e = events[i]
      const d = e.data
      if (e.type === 'user/message') {
        const t = textOf(d && d.content)
        if (t) activity.push({ time: e.time, kind: 'user', html: renderMarkdown(clip(t, 600)) })
      } else if (e.type === 'assistant/message') {
        const t = textOf(d && d.message && d.message.content)
        if (t) activity.push({ time: e.time, kind: 'assistant', html: renderMarkdown(clip(t, 900)) })
      } else if (e.type === 'tool/call') {
        activity.push({ time: e.time, kind: 'tool', html: '<code>' + escapeHtml((d && d.name) || '工具') + '</code>' })
      }
    }
    activity.reverse()
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        json(res, 200, { sessions: await views() })
      } catch (err) {
