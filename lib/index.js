...
function bindThumbs(){var el=document.getElementById('activity');var th=el.querySelectorAll('.turn.thumb');for(var i=0;i<th.length;i++){th[i].addEventListener('click',function(){var idx=parseInt(this.getAttribute('data-i'),10);if(expanded[idx]){delete expanded[idx];}else{expanded[idx]=true;}paintActivity();});}}
function paintActivity(){var el=document.getElementById('activity');var prev=el.scrollTop;el.innerHTML=buildActivityHTML();el.scrollTop=Math.min(prev,el.scrollHeight-el.clientHeight);bindThumbs();}
function buildJumpList(){var el=document.getElementById('jumplist');var h='';var cnt=0;for(var i=0;i<turnsData.length;i++){if(!turnsData[i].q)continue;cnt++;var p=turnsData[i].u.p||'';var preview=p.length>40?esc(p.slice(0,40))+'…':esc(p);h+='<button class="jumpitem" data-i="'+i+'"><div>'+preview+'</div><div class="jt">'+fmtTime(turnsData[i].u.t)+'</div></button>';}el.innerHTML=h||'<div class="empty">暂无提问</div>';var it=el.querySelectorAll('.jumpitem');for(var j=0;j<it.length;j++){it[j].addEventListener('click',function(){jumpTo(parseInt(this.getAttribute('data-i'),10));});}}
